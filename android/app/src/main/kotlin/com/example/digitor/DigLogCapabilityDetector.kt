...
class DigLogCapabilityDetector(private val context: Context) {
    ...
    fun detect(): Result {
        ...
        for (id in manager.cameraIdList) {
            ...
            // This is only a candidate decision. DigLog V3.1 still validates the
            // combined P010 session, RGB10 EGL surface, Main10 codec configuration,
            // and actual output profile at runtime before reporting 10-bit success.
            if (hasRecordSize && p010 && main10 && Build.VERSION.SDK_INT >= 33 &&
                (level == CameraCharacteristics.INFO_SUPPORTED_HARDWARE_LEVEL_FULL ||
                    level == CameraCharacteristics.INFO_SUPPORTED_HARDWARE_LEVEL_3)) {
                return Result(
                    true, id, 10, "DigLog V3.1 10-bit candidate; runtime validation required", levelName,

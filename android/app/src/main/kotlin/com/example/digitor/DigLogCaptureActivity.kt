...
class DigLogCaptureActivity : Activity() {
    ...
    private var requestedBitDepth = 8
    private var activeBitDepth = 8
    private var activeEngineVersion = "DigLog V4"
    ...
    private fun startRecording() {
        ...
        fallbackReason = null
        requestedBitDepth = if (capability.internalBitDepth == 10) 10 else 8
        if (capability.internalBitDepth == 10) startTenBitRecording() else startEightBitRecording()
    }
    ...
    private fun startTenBitRecording() {
        ...
        try {
            ...
            record.isEnabled = false
            status.text = "DigLog V3.1 validating 10-bit"
            val texture = preview.surfaceTexture ?: throw IllegalStateException("Preview surface unavailable")
            texture.setDefaultBufferSize(previewSize.width, previewSize.height)
            val previewSurface = Surface(texture)
            val engine = DigLogV31Engine(
                camera = device,
                size = videoSize,
                fps = 30,
                bitrate = if (videoSize.width >= 1920) 24_000_000 else 12_000_000,
                output = outputFile!!,
                ...
                onReady = { codec ->
                    ...
                    activeAudio = false
                    activeBitDepth = 10
                    activeEngineVersion = "DigLog V3.1"
                    activeBitrate = if (videoSize.width >= 1920) 24_000_000 else 12_000_000
                    recording = true
                    runOnUiThread { record.isEnabled = true; record.text = "STOP"; status.text = "DigLog • REC" }
                },
                onError = { message ->
                    if (!tenBitFallbackStarted) {
                        tenBitFallbackStarted = true
                        fallbackReason = message
                        outputFile?.delete()
                        runOnUiThread {
                            status.text = "DigLog 8-bit fallback"
    ...
    private fun startEightBitRecording() {
        ...
        try {
            ...
            outputFile = createOutputFile()
            record.isEnabled = false
            status.text = "DigLog V4 preparing"

            val texture = preview.surfaceTexture ?: throw IllegalStateException("Preview surface unavailable")
            texture.setDefaultBufferSize(previewSize.width, previewSize.height)
            val previewSurface = Surface(texture)
            val engine = DigLogV4Engine(
                camera = device,
                size = videoSize,
                fps = 30,
    ...
    private fun writeMetadata(file: File): File {
        val json = JSONObject().apply {
            put("profile", "DigLog")
            put("engineVersion", activeEngineVersion)
            put("requestedBitDepth", requestedBitDepth)
            put("actualBitDepth", activeBitDepth)
            put("transferFunction", "DigLog")
            put("capture", if (activeBitDepth == 10) "Camera2 preview + P010 ImageReader → 16-bit integer GL textures → RGB10 EGL encoder surface → HEVC Main10" else "Camera2 preview surface + YUV_420_888 ImageReader → OpenGL ES DigLog transform → encoder-only EGL surface")

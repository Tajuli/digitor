...
class ColorWheelControl {
  const ColorWheelControl({this.chroma = Offset.zero, this.luminance = 0});

  final Offset chroma;
  final double luminance;

  ColorWheelControl copyWith({Offset? chroma, double? luminance}) =>
      ColorWheelControl(chroma: chroma ?? this.chroma, luminance: luminance ?? this.luminance);
  ...
  factory ColorWheelControl.fromJson(Map<String, dynamic> json) => ColorWheelControl(
    chroma: Offset(
...
class ColorWheelSettings {
  const ColorWheelSettings({
    this.previewEnabled = true,
    this.lift = const ColorWheelControl(),
    this.gamma = const ColorWheelControl(),
    this.gain = const ColorWheelControl(),
    this.offset = const ColorWheelControl(),
  });

  final bool previewEnabled;
  final ColorWheelControl lift;
  final ColorWheelControl gamma;
  final ColorWheelControl gain;
  final ColorWheelControl offset;

  ColorWheelSettings copyWith({
    bool? previewEnabled,
    ColorWheelControl? lift,
    ColorWheelControl? gamma,
    ColorWheelControl? gain,
    ColorWheelControl? offset,
  }) => ColorWheelSettings(
  ...
  factory ColorWheelSettings.fromJson(Map<String, dynamic> json) => ColorWheelSettings(
    previewEnabled: json['previewEnabled'] as bool? ?? true,
    lift: ColorWheelControl.fromJson(Map<String, dynamic>.from((json['lift'] as Map?) ?? const {})),
    gamma: ColorWheelControl.fromJson(Map<String, dynamic>.from((json['gamma'] as Map?) ?? const {})),
    gain: ColorWheelControl.fromJson(Map<String, dynamic>.from((json['gain'] as Map?) ?? const {})),
    offset: ColorWheelControl.fromJson(Map<String, dynamic>.from((json['offset'] as Map?) ?? const {})),
  );
}
...
class ColorNode {
  const ColorNode({
    ...
    this.wheels = const ColorWheelSettings(),
  });
  ...
  final ColorWheelSettings wheels;
  ...
  Map<String, dynamic> toJson() => {
    ...
    'grade': {
      ...
      'hue': grade.hue, 'colorBoost': grade.colorBoost,
    },
  ...
  factory ColorNode.fromJson(Map<String, dynamic> json) {
    ...
    return ColorNode(
      ...
      grade: ClipColorAdjustments(
        ...
        hue: (grade['hue'] as num?)?.toDouble() ?? 0,
        colorBoost: (grade['colorBoost'] as num?)?.toDouble() ?? 0,
      ),
      ...
      wheels: ColorWheelSettings.fromJson(Map<String, dynamic>.from((json['wheels'] as Map?) ?? const {})),
    );
  }
}
...
class ColorNodeGraph {
  ...
  ClipColorAdjustments get combinedGrade {
    ...
    for (final node in active) {
      ...
      hue += node.grade.hue;
      colorBoost += node.grade.colorBoost;
    }
    return ClipColorAdjustments(
      ...
      hue: hue.clamp(-1.0, 1.0).toDouble(),
      colorBoost: colorBoost.clamp(-1.0, 1.0).toDouble(),
    );
  }

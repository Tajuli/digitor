...
class _EditorPageState extends State<EditorPage> {
  ...
  void _syncPreviewForTimeline() {
    ...
    final clipChanged =
        _activeClipId != visual.id || _playbackController.sourcePath != path;

    if (clipChanged) {
      _activeClipId = visual.id;
      _playbackController.replaceMedia(path).then((_) async {
        if (!mounted || _activeClipId != visual.id) return;

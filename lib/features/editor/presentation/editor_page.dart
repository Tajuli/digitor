import 'dart:async';
import 'dart:io';
import 'package:digitor/features/editor/application/editor_controller.dart';
import 'package:digitor/features/editor/application/editor_tool_controller.dart';
import 'package:digitor/features/editor/application/timeline_provider.dart';
import 'package:digitor/features/editor/application/project_controller.dart';
import 'package:digitor/features/editor/application/timeline_controller.dart';
import 'package:digitor/features/editor/application/playback_controller.dart';
import 'package:digitor/features/editor/application/preview_proxy_service.dart';
import 'package:digitor/features/editor/application/timeline_audio_playback_controller.dart';
import 'package:digitor/features/editor/application/timeline_render_resolver.dart';
import 'package:digitor/features/editor/application/video_thumbnail_generator.dart';
import 'package:digitor/features/editor/application/media_metadata_service.dart';
import 'package:digitor/features/editor/application/mobile_export_service.dart';
import 'package:digitor/features/editor/domain/models/editor_project.dart';
import 'package:digitor/features/editor/domain/export/export_settings.dart';
import 'package:digitor/features/editor/domain/models/media_item.dart';
import 'package:digitor/features/editor/domain/models/timeline_track.dart';
import 'package:digitor/features/editor/domain/models/timeline_clip.dart';
import 'package:digitor/features/editor/domain/models/clip_type.dart';
import 'package:digitor/features/editor/domain/models/color/color_node_graph.dart';
import 'package:digitor/features/editor/domain/models/track_type.dart';
import 'package:digitor/features/editor/presentation/widgets/editor_toolbar.dart';
import 'package:digitor/features/editor/presentation/color/color_node_sheet.dart';
import 'package:digitor/features/editor/presentation/color/color_grading_sheet.dart';
import 'package:digitor/features/editor/presentation/export/export_sheet.dart';
import 'package:digitor/features/editor/presentation/widgets/preview_area.dart';
import 'package:digitor/features/editor/presentation/timeline/timeline_view.dart';
import 'package:digitor/features/projects/application/project_storage_service.dart';
import 'package:flutter/material.dart';

class EditorPage extends StatefulWidget {
  const EditorPage({
    super.key,
    this.media,
    this.projectId,
    this.projectName,
    this.initialProject,
    this.initialThumbnailPath,
  });

  final MediaItem? media;
  final String? projectId;
  final String? projectName;
  final EditorProject? initialProject;
  final String? initialThumbnailPath;

  @override
  State<EditorPage> createState() => _EditorPageState();
}

class _EditorPageState extends State<EditorPage> {
  late final EditorController _controller;
  late final TimelineProvider _timelineProvider;
  late final ProjectController _projectController;
  late final TimelineController _timelineController;
  late final PlaybackController _playbackController;
  late final PreviewProxyService _previewProxyService;
  late final TimelineAudioPlaybackController _audioPlaybackController;
  late final EditorToolController _toolController;
  final MediaMetadataService _metadataService = VideoPlayerMediaMetadataService();
  final ProjectStorageService _projectStorage = ProjectStorageService();
  String? _activeClipId;
  String? _activeOriginalPath;
  Timer? _transportTimer;
  DateTime? _lastTransportTick;
  DateTime _lastAudioSync = DateTime.fromMillisecondsSinceEpoch(0);
  bool _audioSyncInFlight = false;
  Duration _activeTimelineStart = Duration.zero;
  Duration _activeSourceStart = Duration.zero;
  bool _isSaving = false;
  bool _allowPop = false;
  bool _showColorNodes = false;
  ColorPanelType? _colorPanel;

  @override
  void initState() {
    super.initState();

    _controller = EditorController();
    if (widget.media != null) _controller.loadMedia(widget.media!);

    _timelineProvider = TimelineProvider(
      generator: VideoThumbnailGenerator(),
    );
    _projectController = ProjectController(
      project: widget.initialProject ??
          EditorProject(
            duration: Duration.zero,
            canvasSize: const Size(1920, 1080),
            tracks: [
              TimelineTrack(
                id: 'primary-video',
                name: 'Video 1',
                type: TrackType.video,
              ),
              TimelineTrack(
                id: 'primary-audio',
                name: 'Audio 1',
                type: TrackType.audio,
              ),
            ],
          ),
    );
    _timelineController = TimelineController(projectController: _projectController);
    _playbackController = PlaybackController();
    _previewProxyService = PreviewProxyService();
    _audioPlaybackController = TimelineAudioPlaybackController();
    _toolController = EditorToolController();

    _timelineController.addListener(_syncPreviewForTimeline);
    _projectController.addListener(_syncPreviewForTimeline);
    _playbackController.addListener(_handlePlaybackStateChanged);
    _toolController.addListener(_syncMagnetState);

    if (widget.media != null) _importInitialMedia(widget.media!);

    _loadTimeline();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted && _projectController.hasClips) _syncPreviewForTimeline();
    });
  }

  Future<void> _loadTimeline() async {
    if (widget.media == null || !widget.media!.isVideo) return;

    await _timelineProvider.loadVideo(
      video: File(widget.media!.path),
      duration: widget.media!.duration,
    );
  }

  Future<void> _importInitialMedia(MediaItem media) async {
    if (!media.isVideo) return;
    final metadata = await _metadataService.probeVideo(media.path);
    if (!mounted) return;
    final duration = metadata?.duration ?? media.duration;
    if (duration > Duration.zero) {
      _timelineController.addVideoWithLinkedAudio(
        trackId: 'primary-video',
        path: media.path,
        duration: duration,
        // When stream probing is unavailable, retain the embedded audio path.
        hasAudio: metadata?.hasAudio ?? true,
      );
      _syncPreviewForTimeline();
    }
  }

  void _syncMagnetState() =>
      _timelineController.setMagnetEnabled(_toolController.magnetEnabled);

  void _handlePlaybackStateChanged() {
    if (_playbackController.isPlaying) {
      final duration = _projectController.project.duration;
      if (duration > Duration.zero && _timelineController.position >= duration) {
        _timelineController.setPosition(Duration.zero);
      }
      _startTransportClock();
    } else {
      _stopTransportClock();
    }
    _syncAudioForTimeline();
  }

  void _startTransportClock() {
    if (_transportTimer != null) return;
    _lastTransportTick = DateTime.now();
    _transportTimer = Timer.periodic(
      const Duration(milliseconds: 100),
      (_) => _advanceTransport(),
    );
  }

  void _stopTransportClock() {
    _transportTimer?.cancel();
    _transportTimer = null;
    _lastTransportTick = null;
  }

  void _advanceTransport() {
    if (!_playbackController.isPlaying) {
      _stopTransportClock();
      return;
    }

    final now = DateTime.now();
    final previous = _lastTransportTick ?? now;
    _lastTransportTick = now;
    final delta = now.difference(previous);
    if (delta <= Duration.zero) return;

    final projectDuration = _projectController.project.duration;
    // The global timeline clock must never depend on a single video's decoder
    // position. A decoder stops at its clip boundary, while the project may
    // continue through gaps, audio-only sections, or later video clips.
    final next = _timelineController.position + delta;
    if (projectDuration <= Duration.zero || next >= projectDuration) {
      _timelineController.setPosition(projectDuration);
      _playbackController.pause();
      return;
    }
    _timelineController.setPosition(next);
  }

  void _syncPreviewForTimeline() {
    final timelinePosition = _timelineController.position;
    final visual = TimelineRenderResolver.topVisualClip(
      _projectController.tracks,
      timelinePosition,
    );

    // A visual gap on a higher track must not stop the transport. The resolver
    // already falls through to lower tracks. Only when every visual track is
    // empty at this exact time do we clear the preview and keep the playhead
    // moving until another clip is reached.
    if (visual == null) {
      _activeClipId = null;
      _activeOriginalPath = null;
      _scheduleAudioSync(force: true);
      _playbackController.clearMedia();
      return;
    }

    final path = visual.data['path'] as String?;
    if (path == null || path.isEmpty) return;

    final sourcePosition =
        timelinePosition - visual.start + visual.sourceStart;
    _activeTimelineStart = visual.start;
    _activeSourceStart = visual.sourceStart;
    final clipChanged =
        _activeClipId != visual.id || _activeOriginalPath != path;

    _scheduleAudioSync(force: clipChanged || !_playbackController.isPlaying);

    if (clipChanged) {
      _activeClipId = visual.id;
      _activeOriginalPath = path;
      _loadPreviewClip(
        clipId: visual.id,
        originalPath: path,
        sourcePosition: sourcePosition,
      );
      return;
    }

    // While paused/scrubbing, timeline is authoritative. During playback the
    // decoder runs normally and the independent transport clock drives the
    // global playhead.
    if (!_playbackController.isPlaying) {
      _playbackController.seek(sourcePosition);
    }
  }


  Future<void> _loadPreviewClip({
    required String clipId,
    required String originalPath,
    required Duration sourcePosition,
  }) async {
    // Build a low-resolution proxy once, while preserving the original path in
    // the timeline for full-quality export.
    final previewPath = await _previewProxyService.proxyFor(originalPath);
    if (!mounted || _activeClipId != clipId) return;

    await _playbackController.replaceMedia(previewPath);
    if (!mounted || _activeClipId != clipId) return;

    // Proxy creation can take a while. Seek to the current timeline position,
    // not the stale position captured when proxy generation started.
    final currentVisual = TimelineRenderResolver.topVisualClip(
      _projectController.tracks,
      _timelineController.position,
    );
    final currentSourcePosition = currentVisual?.id == clipId
        ? _timelineController.position -
            currentVisual!.start +
            currentVisual.sourceStart
        : sourcePosition;
    await _playbackController.seek(currentSourcePosition);
  }

  void _syncAudioForTimeline() => _scheduleAudioSync(force: true);

  void _scheduleAudioSync({bool force = false}) {
    final now = DateTime.now();
    if (!force &&
        now.difference(_lastAudioSync) < const Duration(milliseconds: 500)) {
      return;
    }
    if (_audioSyncInFlight) return;

    _audioSyncInFlight = true;
    _lastAudioSync = now;
    final position = _timelineController.position;
    final activeAudioClips = TimelineRenderResolver.activeAudioClips(
      _projectController.tracks,
      position,
    );
    _audioPlaybackController
        .sync(
          activeClips: activeAudioClips,
          timelinePosition: position,
          shouldPlay: _playbackController.isPlaying,
        )
        .whenComplete(() => _audioSyncInFlight = false);
  }

  @override
  void didUpdateWidget(covariant EditorPage oldWidget) {
    super.didUpdateWidget(oldWidget);

    if (oldWidget.media != widget.media && widget.media != null) {
      _controller.loadMedia(widget.media!);
      _loadTimeline();
      _importInitialMedia(widget.media!);
    }
  }


  Future<void> _permitAndPop() async {
    if (!mounted) return;
    setState(() => _allowPop = true);
    await WidgetsBinding.instance.endOfFrame;
    if (mounted) Navigator.of(context).pop();
  }

  Future<void> _saveAndClose() async {
    if (_isSaving) return;
    final projectId = widget.projectId;
    if (projectId == null) {
      await _permitAndPop();
      return;
    }

    setState(() => _isSaving = true);
    try {
      await _projectStorage.saveProject(
        id: projectId,
        name: widget.projectName ?? 'Untitled Project',
        project: _projectController.project,
        previousThumbnailPath: widget.initialThumbnailPath,
      );
      await _permitAndPop();
    } catch (error) {
      if (!mounted) return;
      setState(() => _isSaving = false);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Project could not be saved: $error')),
      );
    }
  }




  Future<void> _openColorPanel(ColorPanelType type) async {
    final clip = _projectController.selectedClip;
    if (clip == null || clip.type != ClipType.video || clip.colorNodeGraph.selectedProcessingNode == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Select a video clip and processing node first.')),
      );
      return;
    }
    _playbackController.pause();
    setState(() {
      _showColorNodes = false;
      _colorPanel = type;
    });
  }

  void _previewColorGraph(ColorNodeGraph graph) {
    final clip = _projectController.selectedClip;
    if (clip == null || clip.type != ClipType.video) return;
    _timelineController.updateClip(clip.copyWith(
      colorNodeGraph: graph,
      colorAdjustments: graph.combinedGrade,
    ));
  }

  void _closeColorPanel(ColorNodeGraph graph) {
    final clip = _projectController.selectedClip;
    if (clip != null && clip.type == ClipType.video) {
      _timelineController.updateClip(clip.copyWith(
        colorNodeGraph: graph,
        colorAdjustments: graph.combinedGrade,
      ));
    }
    if (mounted) setState(() => _colorPanel = null);
  }

  Future<void> _openColorNodes() async {
    final clip = _projectController.selectedClip;
    if (clip == null || clip.type != ClipType.video) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Select a video clip first.')),
      );
      return;
    }
    _playbackController.pause();
    setState(() { _colorPanel = null; _showColorNodes = true; });
  }

  void _closeColorNodes(ColorNodeGraph graph) {
    final clip = _projectController.selectedClip;
    if (clip != null && clip.type == ClipType.video) {
      _timelineController.updateClip(clip.copyWith(
        colorNodeGraph: graph,
        colorAdjustments: graph.combinedGrade,
      ));
    }
    if (mounted) setState(() => _showColorNodes = false);
  }

  Future<void> _openExportSheet() async {
    _playbackController.pause();
    final settings = await showExportSheet(
      context: context,
      project: _projectController.project,
      projectName: widget.projectName ?? 'Digitor Export',
    );
    if (!mounted || settings == null) return;

    final exportService = MobileExportService();
    var cancelled = false;
    final progress = ValueNotifier<int>(0);
    Timer? poller;

    final exportFuture = exportService.export(
      project: _projectController.project,
      settings: settings,
    );

    poller = Timer.periodic(const Duration(milliseconds: 450), (_) async {
      try {
        final value = await exportService.progress();
        progress.value = value.percent;
      } catch (_) {}
    });

    if (!mounted) return;
    showDialog<void>(
      context: context,
      barrierDismissible: false,
      builder: (dialogContext) => AlertDialog(
        title: const Text('Exporting video'),
        content: ValueListenableBuilder<int>(
          valueListenable: progress,
          builder: (context, percent, _) => Column(
            mainAxisSize: MainAxisSize.min,
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              LinearProgressIndicator(value: percent > 0 ? percent / 100 : null),
              const SizedBox(height: 14),
              Text(percent > 0 ? '$percent% complete' : 'Preparing mobile encoder…'),
              const SizedBox(height: 6),
              Text(
                '${settings.resolutionLabel} • ${settings.frameRate} fps • ${settings.codecLabel}',
                style: Theme.of(context).textTheme.bodySmall,
              ),
            ],
          ),
        ),
        actions: [
          TextButton(
            onPressed: () async {
              cancelled = true;
              await exportService.cancel();
              if (dialogContext.mounted) Navigator.pop(dialogContext);
            },
            child: const Text('Cancel'),
          ),
        ],
      ),
    );

    try {
      final location = await exportFuture;
      poller.cancel();
      if (!mounted || cancelled) {
        progress.dispose();
        return;
      }
      Navigator.of(context, rootNavigator: true).pop();
      progress.dispose();
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Export complete: $location')),
      );
    } catch (error) {
      poller.cancel();
      if (!mounted || cancelled) {
        progress.dispose();
        return;
      }
      Navigator.of(context, rootNavigator: true).pop();
      progress.dispose();
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Export failed: $error')),
      );
    }
  }

  @override
  void dispose() {
    _timelineController.removeListener(_syncPreviewForTimeline);
    _projectController.removeListener(_syncPreviewForTimeline);
    _playbackController.removeListener(_handlePlaybackStateChanged);
    _stopTransportClock();
    _toolController.removeListener(_syncMagnetState);
    _timelineProvider.dispose();
    _timelineController.dispose();
    _audioPlaybackController.dispose();
    _playbackController.dispose();
    _previewProxyService.dispose();
    _toolController.dispose();
    _projectController.dispose();
    _controller.dispose();
    super.dispose();
  }

  @override
  Widget build(BuildContext context) {
    final theme = Theme.of(context);

    return PopScope(
      canPop: _allowPop,
      onPopInvokedWithResult: (didPop, result) {
        if (!didPop) _saveAndClose();
      },
      child: Scaffold(
      body: SafeArea(
        child: Column(
          children: [
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 8),
              child: Row(
                children: [
                  IconButton(
                    tooltip: 'Back',
                    onPressed: _isSaving ? null : _saveAndClose,
                    icon: _isSaving
                        ? const SizedBox(
                            width: 20,
                            height: 20,
                            child: CircularProgressIndicator(strokeWidth: 2),
                          )
                        : const Icon(Icons.arrow_back_rounded),
                  ),
                  Expanded(
                    child: Text(
                      widget.projectName ?? 'Editor',
                      textAlign: TextAlign.center,
                      style: theme.textTheme.titleLarge,
                    ),
                  ),
                  IconButton(
                    tooltip: 'More',
                    onPressed: () {},
                    icon: const Icon(Icons.more_vert_rounded),
                  ),
                ],
              ),
            ),
            Expanded(
              child: LayoutBuilder(
                builder: (context, constraints) {
                  return Padding(
                    padding: const EdgeInsets.fromLTRB(0, 0, 0, 8),
                      child: Column(
                          children: [
                            Expanded(
                              flex: constraints.maxHeight < 560 ? 3 : 5,
                              child: ListenableBuilder(
                                listenable: Listenable.merge([_controller, _projectController, _timelineController]),
                                builder: (context, _) {
                                  final session = _controller.session;
                                  final hasTimelineVideo = _projectController.tracks
                                      .where((track) => track.type == TrackType.video)
                                      .expand((track) => track.clips)
                                      .any((clip) => clip.type == ClipType.video);

                                  if (!_projectController.hasClips && session == null) {
                                    final canvas = _projectController.project.canvasSize;
                                    return EmptyPreviewArea(
                                      aspectRatio: canvas.width / canvas.height,
                                    );
                                  }

                                  return PreviewArea(
                                    session: session,
                                    playbackController: _playbackController,
                                    hasTimelineVideo: hasTimelineVideo,
                                    projectDuration: _projectController.project.duration,
                                    timelinePosition: _timelineController.position,
                                    canvasSize: _projectController.project.canvasSize,
                                    previewProxyService: _previewProxyService,
                                    colorNodeGraph: _projectController.selectedClip?.colorNodeGraph,
                                  );
                                },
                              ),
                            ),
                            const SizedBox(height: 8),
                            if (_showColorNodes)
                              Expanded(
                                flex: constraints.maxHeight < 560 ? 4 : 4,
                                child: ColorNodeSheet(
                                  key: ValueKey(_projectController.selectedClip?.id),
                                  graph: _projectController.selectedClip!.colorNodeGraph,
                                  embedded: true,
                                  onDone: _closeColorNodes,
                                ),
                              )
                            else if (_colorPanel != null)
                              Expanded(
                                flex: constraints.maxHeight < 560 ? 4 : 4,
                                child: ColorGradingSheet(
                                  key: ValueKey('${_projectController.selectedClip?.id}-${_colorPanel!.name}'),
                                  graph: _projectController.selectedClip!.colorNodeGraph,
                                  type: _colorPanel!,
                                  onDone: _closeColorPanel,
                                  onChanged: _previewColorGraph,
                                ),
                              )
                            else ...[
                              Expanded(
                                flex: constraints.maxHeight < 560 ? 4 : 4,
                                child: TimelineView(
                                  controller: _projectController,
                                  timelineController: _timelineController,
                                  playbackController: _playbackController,
                                ),
                              ),
                              const SizedBox(height: 8),
                              SizedBox(
                                height: 128,
                                child: EditorToolbar(
                                  tools: _toolController,
                                  project: _projectController,
                                  timeline: _timelineController,
                                  onExport: _openExportSheet,
                                  onOpenNodes: _openColorNodes,
                                  onOpenCorrection: () => _openColorPanel(ColorPanelType.correction),
                                  onOpenColorWheels: () => _openColorPanel(ColorPanelType.wheels),
                                  onOpenCurves: () => _openColorPanel(ColorPanelType.curves),
                                  onOpenQualifier: () => _openColorPanel(ColorPanelType.qualifier),
                                ),
                              ),
                            ],
                          ],
                        
                    ),
                  );
                },
              ),
            ),
          ],
        ),
      ),
    ),
    );
  }
}

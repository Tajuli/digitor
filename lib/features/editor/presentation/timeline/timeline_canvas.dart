...
class TimelineCanvas extends StatefulWidget { const TimelineCanvas({super.key, required this.projectController, required this.timelineController, required this.scrollController}); final ProjectController projectController; final TimelineController timelineController; final TimelineScrollController scrollController; @override State<TimelineCanvas> createState() => _TimelineCanvasState(); }
class _TimelineCanvasState extends State<TimelineCanvas> {
  Timer? _autoScroll;
  @override void dispose() { _autoScroll?.cancel(); super.dispose(); }
  @override Widget build(BuildContext context) { final project = widget.projectController.project; final width = (project.duration.inMilliseconds / 1000 * widget.timelineController.pixelsPerSecond).clamp(MediaQuery.sizeOf(context).width, double.infinity).toDouble(); final playheadX = widget.timelineController.position.inMilliseconds / 1000 * widget.timelineController.pixelsPerSecond;
    return Listener(onPointerMove: (event) => _autoScrollAt(event.position.dx), onPointerUp: (_) => _autoScroll?.cancel(), child: SingleChildScrollView(controller: widget.scrollController.horizontal, scrollDirection: Axis.horizontal, child: SizedBox(width: width, child: Stack(children: [Column(children: [TimeRuler(duration: project.duration, pixelsPerSecond: widget.timelineController.pixelsPerSecond), const Divider(height: 1), TimelineBody(tracks: project.tracks, width: width, projectController: widget.projectController, timelineController: widget.timelineController, onMoveEnd: _moveEnd)]), Playhead(x: playheadX, height: TimelineConstants.rulerHeight + 1 + project.tracks.length * TimelineConstants.trackHeight, position: widget.timelineController.position)])))); }

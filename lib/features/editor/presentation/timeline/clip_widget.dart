      ...
      child: GestureDetector(
        ...
        child: RepaintBoundary(child: AnimatedContainer(
          ...
          child: Stack(children: [
            if (widget.clip.type == ClipType.audio) _AudioWaveform(clip: widget.clip, pixelsPerSecond: widget.pixelsPerSecond),
...
class _AudioWaveform extends StatelessWidget {
  const _AudioWaveform({required this.clip, required this.pixelsPerSecond});
  ...
  Widget build(BuildContext context) {
    ...
    final visibleWidth = TimelineMath.durationToPixels(clip.duration, pixelsPerSecond);
    final samples = visibleWidth.clamp(240.0, 4000.0).round();
    return Positioned.fill(
      child: ClipRRect(
        borderRadius: BorderRadius.circular(TimelineConstants.clipRadius),
        child: FutureBuilder<List<double>>(
          future: AudioWaveformCache.peaksFor(path, samples: samples),
...
class _AudioWaveformPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final paint = Paint()..color = color..strokeWidth = 1.15..strokeCap = StrokeCap.round;
    final centerPaint = Paint()..color = color.withOpacity(.25)..strokeWidth = .7;
    final center = size.height / 2;
    canvas.drawLine(Offset(0, center), Offset(size.width, center), centerPaint);
    final visibleSamples = math.max(1, size.width.floor());
    final step = size.width >= 900 ? 1 : 2;
    for (var x = 0; x < visibleSamples; x += step) {
      final index = (x / visibleSamples * peaks.length).floor().clamp(0, peaks.length - 1).toInt();
      final amplitude = peaks[index] * size.height * .44;

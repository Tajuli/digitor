...
class _PreviewAreaState extends State<PreviewArea> {
  ...
  Widget _interactiveMedia(Widget media) {
    final graded = widget.colorNodeGraph == null
        ? media
        : ColorGradeFilter(graph: widget.colorNodeGraph!, child: media);
    return ClipRect(
        child: InteractiveViewer(
          ...
          child: RepaintBoundary(child: graded),
        ),
      );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return _PreviewFrame(
      aspectRatio: widget.canvasSize.width / widget.canvasSize.height,
      child: Stack(
        fit: StackFit.expand,
        children: [
          content,
...
class EmptyPreviewArea extends StatelessWidget {
  const EmptyPreviewArea({super.key, this.aspectRatio = 16 / 9});
  ...
  Widget build(BuildContext context) => _PreviewFrame(
        ...
        child: const EmptyPreviewContent(),
      );
}
...
class _PreviewFrame extends StatelessWidget {
  const _PreviewFrame({required this.child, required this.aspectRatio});

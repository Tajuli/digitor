  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        ...
        if (controller == null) {
          // A timeline can legitimately contain gaps or audio-only regions.
          // Show an empty preview instead of an endless loading spinner while
          // the global transport continues toward the next visual clip.
          if (playbackController.sourcePath == null) {
            return _TimelineGapPreview(
              playbackController: playbackController,
              projectDuration: projectDuration ?? Duration.zero,
              timelinePosition: timelinePosition ?? Duration.zero,
            );
          }
...
class _TimelineGapPreview extends StatelessWidget {
  const _TimelineGapPreview({
    required this.playbackController,
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      behavior: HitTestBehavior.opaque,
      onTap: playbackController.toggle,
      child: ColoredBox(
        color: Colors.black,
        child: Stack(
          fit: StackFit.expand,
                ...
                child: Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      const Spacer(),
                      Text(
                        '${_formatTimelineTime(timelinePosition)} / ${_formatTimelineTime(projectDuration)}',
...
String _formatTimelineTime(Duration value) {
  final hours = value.inHours;
  final minutes = value.inMinutes.remainder(60).toString().padLeft(2, '0');
  final seconds = value.inSeconds.remainder(60).toString().padLeft(2, '0');
  return hours > 0 ? '$hours:$minutes:$seconds' : '$minutes:$seconds';

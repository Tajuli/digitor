...
class TimelineRenderResolver {
  ...
  static TimelineClip? topVisualClip(
    ...
    for (final track in tracks.where(
      ...
      final active = track.clips.where(
        (clip) =>
      ...
      if (active.isNotEmpty) {
        // Later clips in the same track are treated as being on top when an
        // overlap exists. Track priority itself is still top-to-bottom.
        return active.last;
      }
    }
    return null;

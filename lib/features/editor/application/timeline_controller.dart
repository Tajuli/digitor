...
class TimelineController extends ChangeNotifier {
  ...
  int get fps => projectController.project.fps;
  double get pixelsPerFrame => pixelsPerSecond / fps;
  bool get framePrecisionEnabled => pixelsPerFrame >= TimelineConstants.minimumPixelsPerFrameForSnap;
  ...
  Duration snapPlayhead(Duration value) {
    var clamped = TimelineMath.clampTimelinePosition(
      value,
      projectController.project.duration,
    );

    // At high zoom every playhead position is quantized to an exact frame.
    // This prevents the cursor from landing between frames at maximum zoom.
    if (framePrecisionEnabled) {
      clamped = TimelineMath.clampTimelinePosition(
        TimelineMath.snapToFrame(clamped, fps),
        projectController.project.duration,
      );
    }
  ...
  Duration snap(Duration value, {String? excludingClipId}) {
    var normalized = value < Duration.zero ? Duration.zero : value;
    if (framePrecisionEnabled) {
      normalized = TimelineMath.snapToFrame(normalized, fps);
    }
    if (!_magnetEnabled) return normalized;
    ...
    final nearest = candidates.reduce((a, b) => (a - normalized).abs() < (b - normalized).abs() ? a : b);
    final threshold = TimelineMath.pixelsToDuration(
      TimelineConstants.snapThresholdPixels,
      pixelsPerSecond,
    );
    return (nearest - normalized).abs() <= threshold ? nearest : normalized;
  }

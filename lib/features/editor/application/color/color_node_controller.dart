...
class ColorNodeController extends ChangeNotifier {
  ...
  void deleteNode(String id) {
    ...
    if (node.type == ColorNodeType.parallel && outgoing.length == 1) {
      ...
      if (mixer?.type == ColorNodeType.parallelMixer) {
        final remainingInputs = connections
            .where((connection) => connection.to == mixer!.id)
            .toList();
        final mixerOutputs = connections
            .where((connection) => connection.from == mixer!.id)
            .toList();
        if (remainingInputs.length <= 1 && mixerOutputs.length == 1) {
          ...
          connections = connections
              .where(
                (connection) =>
                    connection.from != mixer.id && connection.to != mixer.id,
              )
              .toList();
          nodes = nodes.where((item) => item.id != mixer.id).toList();
          if (remaining != null) {
            connections.add(

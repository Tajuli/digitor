...
class TimelineAudioPlaybackController {
  ...
  final Map<String, double> _lastVolumes = <String, double>{};
  ...
  Future<void> sync({
    ...
    for (final clip in activeClips) {
      ...
      if (player == null &&
          !_loading.contains(clip.id) &&
          !_failed.contains(clip.id)) {
      ...
      final targetVolume =
          clip.muted ? 0.0 : clip.volume.clamp(0.0, 1.0).toDouble();
      if (_lastVolumes[clip.id] != targetVolume) {
        await player.setVolume(targetVolume);
        _lastVolumes[clip.id] = targetVolume;
      }
      ...
      if (shouldPlay && !player.playing) {
        ...
        unawaited(
          player.play().catchError((Object error, StackTrace stackTrace) {
            debugPrint('Unable to play audio clip ${clip.id}: $error');
            debugPrintStack(stackTrace: stackTrace);
          }),
        );
